        ...
        materials.forEach(material => {
            material.dispose();
             Object.keys(material).forEach(key => {
                ...
                if (matProp && typeof matProp === 'object' && 'dispose' in matProp && typeof (matProp as any).dispose === 'function') {
                    (matProp as any).dispose();
                }
            });
        });
      }
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    // Use a short delay to ensure layout is stable
    ...
    >
      <style>{`
        .ascii-text-container canvas {
          ...
          display: none; /* Hide the webgl canvas, we only need the pre */
        }
        ...
        .ascii-text-container pre {
          ...
          left: 0;
          top: 0;
          background-image: radial-gradient(circle, hsl(var(--primary)) 0%, hsl(var(--accent)) 100%);
          ...
          color: white; /* Fallback for browsers that dont support background-clip */
        }
      `}</style>
    </div>
  );
